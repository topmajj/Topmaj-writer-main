...
export async function POST(req: Request) {
  try {
    ...
    logger.info(`Received Fatora form submission request for user ${userId}, plan ${planName}`)
    ...
    // Create an HTML form that will auto-submit to Fatora
    const html = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <h2>Redirecting to Fatora Payment...</h2>
        <p>Please wait while we redirect you to the payment page.</p>
        <div class="loader"></div>
        
        <form id="fatoraForm" action="https://api.fatora.io/v1/pay" method="POST">
    ...
    logger.error(`Fatora form submission error: ${error.message}`)
    return NextResponse.json({ error: "Failed to create form submission", details: error.message }, { status: 500 })
  }
}
