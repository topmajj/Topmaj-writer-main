...
export async function GET(req: Request) {
  try {
    ...
    const testOrderId = `test_no_name_${Date.now()}`

    // Create a payload without the name field
    const payload = {
      amount: 1.0,
      currency: "QAR",
    ...
    console.log("Sending request to Fatora API without name field:", JSON.stringify(payload))
